import { size, colors, white, black, noFig, markFactor, noPos, } from "./statics.js";
let isLogOn = false;
function log(text) {
    if (isLogOn)
        console.log(text);
...
export function startGame(playerBlack, playerWhite, logON) {
    isLogOn = logON;
    ...
    while (!isGameOver(board)) {
        ...
        let move;
        if (turn == black)
            move = playerBlack.getMove(board);
        else
            move = playerWhite.getMove(board);
        log("move: " + move);
...
function getStringWithSpaces(text1) {
    const minLen = 3;
...
export function getStartBoard() {
    ...
    for (let x = 0; x < size; x++) {
        ...
        for (let y = 0; y < size; y++) {
            if (x === 0)
                board[x][y] = white * colors[x][y].code;
            else if (x === size - 1)
                board[x][y] = black * colors[x][y].code;
            else
                board[x][y] = noFig;
        }
    }
...
function isPlayerOnWinLine(board) {
    for (let y = 0; y < size; y++) {
        if (board[0][y] < 0 || board[size - 1][y] > 0)
            return true;
    }
    return false;
}
function isBlockCycle(board1) {
...
function getMarkPos(board) {
    for (let x = 0; x < size; x++) {
        for (let y = 0; y < size; y++) {
            if (Math.abs(board[x][y]) > size)
                return [x, y];
        }
    }
    return noPos;
}
function getMarkedFig(board) {
    const markPos = getMarkPos(board);
    if (markPos === noPos)
        return noFig;
    return board[markPos[0]][markPos[1]];
}
function isBlocked(board) {
...
export function getBoardCopy(board1) {
    const board2 = [];
    for (let x = 0; x < size; x++) {
        board2[x] = [];
        for (let y = 0; y < size; y++) {
...
function isMovePossible(move, board) {
...
function isMoveIncluded(move, moves) {
    for (let i = 0; i < moves.length; i++) {
...
export function doMove(target, board) {
